...
class PCNA_Viewer(napari.Viewer):
    ...
    def __init__(self, track_path, mask_path, 
                 frame_base=1, pcna_path=None, bf_path=None, comp_path=None,
                 title='pcnaDeep', ndisplay=2, order=(),
                 axis_labels=(), show=True, sat=1, gamma=1):
        """
        To correct track ID, mitosis relationship, cell cycle classifications.

        Args:
            track_path (str): path to tracked object table.
            comp_path (str): composite image. The PCNA channel must be the first while BF the last.
                If `comp_path` is supplied, will ignore `pcna/bf_path`, otherwise images of PCNA and BF should be 
                supplied through `pcna/bf_path` correspondingly.
            pcna_path (str): path to PCNA channel tif image.
            bf_path (str): path to bright field channel tif image.
            mask_path (str): segmentatio mask.
            frame_base (int): base of counting frames, default 1.
            sat (float): saturation of pixel to rescale PCNA and BF image. No apply to composite.
            gamma (float): gamma factor to rescale PCNA image. No apply to composite.
        """
        ...
        elif pcna_path is not None and bf_path is not None:
            comp = getDetectInput(io.imread(pcna_path), io.imread(bf_path), sat=sat, gamma=gamma)
        else:
            raise ValueError('No image input!')
        self.add_image(comp[:,:,:,0], name='pcna')
        self.add_image(comp[:,:,:,-1], name='bf')
    ...
    def __setattr__(self, key, value):
        if key not in ['track_path', 'track', 'saved', 'original', 'frame_base', 'parser', 'track_count',
                       'text_config', 'mask', 'mask_path', 'comp_path', 'dic_path', 'pcna_path']:
    ...
    def correct_cls(self, trk_id, frame, cls, mode='to_next', end_frame=None):
        ...
        msg = 'Track ' + str(trk_id) + ' phase <- ' + str(cls) + ' from ' + \
    ...
    def register_obj(self, obj_id, frame, trk_id, cls):
        ...
        self.track = self.track.sort_values(by=['trackId', 'frame'])
        msg = 'New obj: track ' + str(trk_id) + '; frame ' + str(frame) + '; phase ' + cls + '.'
        return msg
